...
1. **Read first**: [`references/detection.md`](skills/harness-sync/references/detection.md) and [`references/sync-strategies.md`](skills/harness-sync/references/sync-strategies.md). Then the closest existing harness (`codex.md` for symlink+translate, `gemini.md` for pure translate).
...
CI (`.github/workflows/ci.yml`) runs `py_compile` and the smoke test on Python 3.11–3.13. Keep both green.
